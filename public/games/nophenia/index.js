  ...
  return async (moduleArg = {}) => {
    ...
    var _environ_get = (__environ, environ_buf) => {
      ...
      for (var string of getEnvStrings()) {
        ...
        bufSize += stringToUTF8(string, ptr, Number.POSITIVE_INFINITY) + 1;
        envp += 4;
      }
      return 0;
    };
    ...
    var jstoi_q = (str) => Number.parseInt(str);
    ...
    var GodotRuntime = {
      get_func: (ptr) => wasmTable.get(ptr),
      error: () => {
      ...
      malloc: (p_size) => _malloc(p_size),
      free: (p_ptr) => {
        _free(p_ptr);
      },
      getHeapValue: (p_ptr, p_type) => getValue(p_ptr, p_type),
      ...
      parseString: (p_ptr) => UTF8ToString(p_ptr),
      ...
      strlen: (p_str) => lengthBytesUTF8(p_str),
      allocString: (p_str) => {
      ...
      stringToHeap: (p_str, p_ptr, p_len) =>
        stringToUTF8Array(p_str, HEAP8, p_ptr, p_len),
    };
    var GodotConfig = {
      canvas: null,
      locale: 'en',
      ...
      locate_file: (file) => Module['locateFile'](file),
      clear: () => {
    ...
    var GodotFS = {
      ...
      _syncing: false,
      _mount_points: [],
      is_persistent: () => (GodotFS._idbfs ? 1 : 0),
      init: (persistentPaths) => {
        ...
        GodotFS._mount_points.forEach((path) => {
    ...
    var GodotOS = {
      ...
      finish_async: (callback) => {
        GodotOS._fs_sync_promise
          .then((err) => {
            const promises = [];
            GodotOS._async_cbs.forEach((cb) => {
    ...
    var GodotAudio = {
            ...
            switch (event.data['type']) {
              case 'position':
                this._playbackPosition =
                  Number.parseInt(event.data.data, 10) /
      ...
      linear_to_db: (linear) => Math.log(linear) * 8.685889638065037,
      db_to_linear: (db) => Math.exp(db * 0.11512925464970228),
      init: (mix_rate, latency, onstatechange, onlatencyupdate) => {
      ...
      create_input: (callback) => {
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          ...
          navigator.getUserMedia({ audio: true }, gotMediaInput, (e) => {
            GodotRuntime.print(e);
          });
        }
        return 0;
      },
      close_async: (resolve, reject) => {
      ...
      stop_sample: (playbackObjectId) => {
        GodotAudio.SampleNode.stopSampleNode(playbackObjectId);
      },
      sample_set_pause: (playbackObjectId, pause) => {
        GodotAudio.SampleNode.pauseSampleNode(playbackObjectId, pause);
      },
      update_sample_pitch_scale: (playbackObjectId, pitchScale) => {
        const sampleNode =
      ...
      set_sample_bus_count: (count) => {
        GodotAudio.Bus.setCount(count);
      },
      remove_sample_bus: (index) => {
      ...
      add_sample_bus: (atPos) => {
        GodotAudio.Bus.addAt(atPos);
      },
      move_sample_bus: (busIndex, toPos) => {
        GodotAudio.Bus.move(busIndex, toPos);
      },
      set_sample_bus_send: (busIndex, sendIndex) => {
      ...
      set_sample_bus_volume_db: (busIndex, volumeDb) => {
        const bus = GodotAudio.Bus.getBusOrNull(busIndex);
        if (bus == null) {
          return;
        }
      ...
      set_sample_bus_solo: (busIndex, enable) => {
        const bus = GodotAudio.Bus.getBusOrNull(busIndex);
        if (bus == null) {
          return;
        }
      ...
      set_sample_bus_mute: (busIndex, enable) => {
        const bus = GodotAudio.Bus.getBusOrNull(busIndex);
        if (bus == null) {
          return;
        }
    ...
    function _godot_audio_input_start() {
      return GodotAudio.create_input((input) => {
    ...
    var GodotAudioScript = {
      script: null,
      create: (buffer_length, channel_count) => {
      ...
      start: (p_in_buf, p_in_size, p_out_buf, p_out_size, onprocess) => {
        GodotAudioScript.script.onaudioprocess = (event) => {
      ...
      get_node: () => GodotAudioScript.script,
      close: () =>
    ...
    var GodotAudioWorklet = {
      ...
      get_node: () => GodotAudioWorklet.worklet,
      close: () =>
        new Promise((resolve, reject) => {
    ...
    var GodotDisplayCursor = {
      ...
      clear: () => {
        ...
        Object.keys(GodotDisplayCursor.cursors).forEach((key) => {
      ...
      isPointerLocked: () => document.pointerLockElement === GodotConfig.canvas,
    };
    var GodotEventListeners = {
      handlers: [],
      has: (target, event, method, capture) =>
        GodotEventListeners.handlers.findIndex(
          (e) =>
            e.target === target &&
            e.event === event &&
            e.method === method &&
      ...
      add: (target, event, method, capture) => {
      ...
      clear: () => {
        GodotEventListeners.handlers.forEach((h) => {
    ...
    var GodotDisplayScreen = {
      ...
      hidpi: true,
      getPixelRatio: () =>
        GodotDisplayScreen.hidpi ? window.devicePixelRatio || 1 : 1,
      isFullscreen: () => {
        const elem =
    ...
    var GodotDisplayVK = {
      textinput: null,
      textarea: null,
      available: () => GodotConfig.virtual_keyboard && 'ontouchstart' in window,
      init: (input_cb) => {
      ...
      hide: () => {
        ...
        [GodotDisplayVK.textinput, GodotDisplayVK.textarea].forEach((elem) => {
          elem.blur();
          elem.style.display = 'none';
          elem.value = '';
        });
    ...
    function _godot_js_display_notification_cb(
      ...
      ['mouseover', 'mouseleave', 'focus', 'blur'].forEach((evt_name, idx) => {
        GodotEventListeners.add(
          canvas,
          evt_name,
          () => {
    ...
    var IDHandler = {
      _last_id: 0,
      _references: {},
      get: (p_id) => IDHandler._references[p_id],
      add: (p_data) => {
    ...
    var GodotInputGamepads = {
      ...
      get_samples: () => GodotInputGamepads.samples,
      get_sample: (index) => {
    ...
    var GodotInputDragDrop = {
      ...
      process: (resolve, reject) => {
        ...
        GodotInputDragDrop.promises.pop().then(() => {
          setTimeout(() => {
            GodotInputDragDrop.process(resolve, reject);
          }, 0);
        });
      },
      _process_event: (ev, callback) => {
        ev.preventDefault();
        ...
        new Promise(GodotInputDragDrop.process).then(() => {
          const DROP = `/tmp/drop-${Number.parseInt(Math.random() * (1 << 30), 10)}/`;
          const drops = [];
          const files = [];
    ...
    function _godot_js_os_fs_sync(callback) {
      ...
      GodotOS._fs_sync_promise.then((err) => {
        func();
      });
    }
    ...
    var GodotPWA = {
      ...
      updateState: (cb, reg) => {
        ...
        GodotEventListeners.add(reg, 'updatefound', () => {
    ...
    function _godot_js_rtc_pc_local_description_set(
      ...
      ref.setLocalDescription({ sdp, type }).catch((error) => {
    ...
    var GodotJSWrapper = {
      ...
      isBuffer: (obj) => obj instanceof ArrayBuffer || ArrayBuffer.isView(obj),
    };
      ...
      navigator.xr
        ...
        .then((session) => {
          ...
          ['selectstart', 'selectend', 'squeezestart', 'squeezeend'].forEach(
            (input_event, index) => {
              session.addEventListener(input_event, (evt) => {
    ...
    function _godot_webxr_is_session_supported(p_session_mode, p_callback) {
      ...
      if (navigator.xr) {
        navigator.xr.isSessionSupported(session_mode).then((supported) => {
    ...
    function assignWasmExports(wasmExports) {
      ...
      Module['__Z14godot_web_mainiPPc'] = __Z14godot_web_mainiPPc =
        createExportWrapper('_Z14godot_web_mainiPPc', 2);
      Module['_main'] = _main = createExportWrapper('__main_argc_argv', 2);
      Module['_malloc'] = _malloc = createExportWrapper('malloc', 1);
      _fflush = createExportWrapper('fflush', 1);
      Module['__emwebxr_on_input_event'] = __emwebxr_on_input_event =
        createExportWrapper('_emwebxr_on_input_event', 2);
      Module['__emwebxr_on_simple_event'] = __emwebxr_on_simple_event =
...
const Preloader = /** @constructor */ function () {
  ...
  const animateProgress = () => {
    ...
    Object.keys(loadingFiles).forEach((file) => {
 ...
 * @ignore
 */
const InternalConfig = (initConfig) => {
  // eslint-disable-line no-unused-vars
